      ...
      <div className="relative z-10 flex flex-col items-center gap-8 w-80">
        <div className="relative">
          ...
          <div className="absolute inset-0 flex items-center justify-center">
             {step < 3 ? <Fingerprint size={48} className="text-emerald-500/80 animate-pulse" /> : <ShieldCheck size={48} className="text-emerald-400 animate-bounce" />}
          </div>
        </div>
        <div className="w-full space-y-2">
          ...
          <div className="h-1 bg-emerald-900/50 rounded-full overflow-hidden">
            <div className="h-full bg-emerald-500 shadow-[0_0_15px_#10b981] transition-all duration-700 ease-out" style={{ width: `${Math.min((step + 1) * 20, 100)}%` }}></div>
          </div>
        </div>
